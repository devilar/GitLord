...
\CBitrixComponent::includeComponentClass("td:catalog.smart.filter");
$smartFilterClass = new CBitrixCatalogSmartFilter();
$sectionPath = $arResult['SECTION']['PATH'][1]?:$arResult['SECTION']['PATH'][0];

$sefSmartUrl = $sectionPath['SECTION_PAGE_URL']  . "filter/#SMART_FILTER_PATH#/";

if(!empty($arResult['DISPLAY_PROPERTIES']))
{
	foreach ($arResult['DISPLAY_PROPERTIES'] as $pCode => $prop)
	{
		if($prop["PROPERTY_TYPE"] != "N" && $prop["DISPLAY_TYPE"] != "U")
		{
			$code = $url_id = null;

			if ($prop["CODE"]){
				$code = toLower($prop["CODE"]);
			}else{
				$code = $prop["ID"];
			}

			$smartFilterClass->fillItemValues($arResult['DISPLAY_PROPERTIES'][$pCode],$prop['VALUE_ENUM_ID']);

			if(is_array($arResult['DISPLAY_PROPERTIES'][$pCode]['VALUES']))
			{
				foreach ($arResult['DISPLAY_PROPERTIES'][$pCode]['VALUES'] as $value)
				{
					$url_id = $value['URL_ID'];
				}

				$propLink = str_replace("#SMART_FILTER_PATH#", implode("/", $smartFilterClass->encodeSmartParts(array(array($code,$url_id)))), $sefSmartUrl);

				$textLink = $arResult['DISPLAY_PROPERTIES'][$pCode]['DISPLAY_VALUE'];

				$arResult['DISPLAY_PROPERTIES'][$pCode]['DISPLAY_VALUE'] = '<a href="'.$propLink.'">'.$textLink.'</a>';
