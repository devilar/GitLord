...
if($arResult["PROPERTIES"]["BREND"]["VALUE"]){
    $arSelect = Array("NAME");
    $arFilter = Array("IBLOCK_ID"=>$arResult["PROPERTIES"]["BREND"]["LINK_IBLOCK_ID"],"ID"=>$arResult["PROPERTIES"]["BREND"]["VALUE"], "ACTIVE"=>"Y");
    $res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, false, $arSelect);
    while($ob = $res->GetNextElement())
    {
        $arResult["BREND"] = $ob->GetFields();
    }
}?>

  <?$arResult["HARAK_PHOTO"] = CFile::GetPath($arResult["PROPERTIES"]["HARAKTERISTIC_PHOTO"]["VALUE"]);?>

  <?$arResult["INSTRUKTION"] = $arResult["DISPLAY_PROPERTIES"]["INSTRUKTION"]["DISPLAY_VALUE"]?>

<?/*if($arResult["PROPERTIES"]["HARAKTERISTIC_PHOTO"]["VALUE"]){
    //print_r($arResult["PROPERTIES"]["HARAKTERISTIC_PHOTO"]["VALUE"]);
    $arSelect = Array("NAME", "ID", "VALUE");
    $arFilter = Array("ID"=>$arResult["ID"]);
    //$arFilter = Array();
    $res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, false, $arSelect);
    while($ob = $res->GetNextElement())
    {
        $arResult["HARAKTERISTIC_PHOTO"] = $ob->GetFields();
        echo '<pre>';
      //print_r($arResult["HARAKTERISTIC_PHOTO"]);
      // echo CFile::GetPath($arResult["HARAKTERISTIC_PHOTO"]["ID"]);
        echo '</pre>';
    }
}*/?>
...
if($page[2] == 'tovary_so_skidkoy_50'){
//        $old_url = explode('/',$arResult["DETAIL_PAGE_URL"]);
//        $arResult["SALE_DETAIL_PAGE_URL"] = '/'.$old_url[1].'/'.$page[2].'/'.$old_url[3].'/';
        foreach($arResult["PRICES"] as $cell=>$arPrice){ // если на товар будет применена скидка мы её не учитываем
            $price = $arPrice["VALUE"];
        }
        $arResult["NEW_SALE_PRICE"] = round(floatval($price*0.5),2).' руб.';

}
if($page[2] == 'tovary_so_skidkoy_80'){
//        $old_url = explode('/',$arResult["DETAIL_PAGE_URL"]);
//        $arResult["SALE_DETAIL_PAGE_URL"] = '/'.$old_url[1].'/'.$page[2].'/'.$old_url[3].'/';
        foreach($arResult["PRICES"] as $cell=>$arPrice){ // если на товар будет применена скидка мы её не учитываем
            $price = $arPrice["VALUE"];
        }
        $arResult["NEW_SALE_PRICE"] = round(floatval($price*0.2),2).' руб.';
}
?>
<?
$cp = $this->__component;
if (is_object($cp))
$cp->SetResultCacheKeys(array('BREND','DETAIL_TEXT','PRICES','NEW_SALE_PRICE','CODE','HARAK_PHOTO','INSTRUKTION'));
?>