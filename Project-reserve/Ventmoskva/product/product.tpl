  ...
  <div class="product-info <?php echo $this->journal2->settings->get('split_ratio'); ?>" data-respond="start: 620px; end: 630px; interval: 10px;">
    ...
    <?php endforeach; ?>
      <div class="description">
        ...
        <?php } ?>
        <span class="p-stock"><?php echo $text_stock; ?></span> <span class="journal-stock"><?php echo $stock_status; ?></span>
      </div>
    ...
    <?php if (isset($date_end) && $date_end && $this->journal2->settings->get('show_countdown_product_page', 'on') == 'on'): ?>
      ...
      <div class="cart <?php echo isset($labels) && is_array($labels) && isset($labels['outofstock']) ? 'outofstock' : ''; ?>">
		...
		<input type="button" value="Быстрый заказ" id="fast_order" class="button button-product-page" onclick="yaCounter36647785.reachGoal('fastorder'); return true;" />
				<div style="display:none">
          <div class="fast-order-form-new" id="fast_order_form"> 
			<?php $url = $_SERVER['REQUEST_URI']; ?>
			<input id="product_href" type="hidden" value="http://ventmoskva.ru<?php echo $url; ?>">
            <input id="product_name" type="hidden" value="<?php echo $heading_title; ?>">
            <input id="customer_sku" type="hidden" value="<?php if (isset($sku)) { ?><?php echo $sku; ?><?php } ?>">
            ...
            <div class="fast_order_left fast_order_image"><img src="<?php echo $thumb; ?>" title="<?php echo $heading_title; ?>" alt="<?php echo $heading_title; ?>" id="image" data-largeimg="<?php echo $popup; ?>" itemprop="image"  /></div>

             <div class="fast_order_right fast_order_price">Цена: <?php echo $price; ?></div>
            ...
            <div class="fast_order_center">
              ...
              <button class="fast_order_button" id="button" onclick="yaCounter25815974.reachGoal('fastorder_sucess'); return true;"><span>Оформить заказ</span></button>
            </div>
          </div>
        </div>
          ...
          <script>
              /* quick buttons */			
				$(document).ready(function () {
					$('#fast_order').colorbox({href:"#fast_order_form",inline:true, width:"650px", height:"410px", title:" "});
		  ...
		  <style>
			...
			#fast_order_form input {margin: 0px;padding: 0px;height: 20px;width: 220px;}
			#fast_order_form .fast_order_button {font-size: 17px;cursor: pointer;height: 40px;width: 220px;}
      #fast_order_form .fast_order_image{width:42%;height:75px;margin-bottom:10px;}
      #fast_order_form .fast_order_price{width:58%;height:75px;line-height:75px;font-size:20px;font-weight:bold;padding-left: 15px;box-sizing: border-box;margin-bottom:10px;}
      #fast_order_form .fast_order_image img{width:auto;height:100%;}
		  </style>
        <?php if ($minimum > 1) { ?>
...
<div id="tab-delivery">
<noindex>  
<div class="delivery-tab-text" style="padding:10px 0;">

<p style="font-weight:bold;font-size:16px;">Способы доставки:</p>
<ul style="line-height:22px;">
<li>
— Самовывоз
Бесплатно
Заказанные товары Вы также можете забрать самостоятельно из нашего обычного магазина, расположенного по адресу:
г. Москва, Новорязанское ш., 25 км, строение 16 , строительный рынок "СтройМаркет", , павильоны 29д и 113а
Режим работы: пн-сб: 08:00-18:00, вс: 08:30-16:00
</li>
...
500 рублей
 ...
 <li>
— Московская область за пределами МКАД
500 руб. + 50 руб./км
</li>
 
<li>
— Транспортные компании
Доставка до отделения Транспортной компании в городе Москва ПЭК-700 руб, Деловые Линии-500руб , КИТ-500 руб, Слон– 700 руб СДЭК-800 РУБ И БАЙКАЛ СЕРВИС -550 РУБ . Услуги перевозки оплачивает получатель.
</li>
